...
### Phase 53: Role Preservation & Null-Safe Security Envelope Injection (June 29, 2026)
- **Null-Safe Security Envelope:** Memperbarui [SessionManager.kt](file:///home/max/azuratime/nusantara-main/app/src/main/java/com/azuratech/azuratime/core/session/SessionManager.kt) dengan me-refactor fungsi `injectSecurityEnvelope` agar memiliki parameter `role` bertipe nullable (`String? = null`). Jika `role` tidak dikirim (misalnya pada pembaruan kunci periodik), fungsi akan mempertahankan role lama pengguna dari database lokal dan tidak menimpanya secara paksa dengan `"USER"`.
- **Explicit Role Transmission:** Mengupdate `activateSession` di [MembershipRepositoryImpl.kt](file:///home/max/azuratime/nusantara-main/app/src/main/java/com/azuratech/azuratime/features/account/data/repo/MembershipRepositoryImpl.kt) untuk secara eksplisit mengekstrak data `role` dari snapshot sesi aktif dan meneruskannya ke `injectSecurityEnvelope`. Hal ini menjamin bahwa role pengguna yang terdaftar di Firestore (seperti `owner` atau `admin`) langsung tersinkronisasi secara akurat ke perangkat saat login pertama kali.
- **Data Leak Prevention:** Menghentikan bug transisi state lokal-first yang secara tidak sengaja mengunggah role `"USER"` yang rusak kembali ke Firestore melalui sinkronisasi latar belakang (`pushAccount`).
- **Verification:** Seluruh source code berhasil dikompilasi (`compileDebugKotlin`) dan semua pengujian unit lokal sukses dijalankan (`testDebugUnitTest`).
